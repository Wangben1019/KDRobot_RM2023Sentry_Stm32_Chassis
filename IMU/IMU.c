		...
		for(uint16_t i = 1; i < 30 ; i+=11)
		{
			switch(Data[i])
			{
                ...
                case kItemAcceleration:
                    Dst->Acceleration.X = ((int16_t)(Data[i+2]<<8)|Data[i+1])/32768*16*9.8;
                    Dst->Acceleration.Y = ((int16_t)(Data[i+4]<<8)|Data[i+3])/32768*16*9.8;
					Dst->Acceleration.Z = ((int16_t)(Data[i+6]<<8)|Data[i+5])/32768*16*9.8;
                break;
#endif
