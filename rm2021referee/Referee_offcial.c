...
ground_robot_position_t ground_robot_position;                     //!<@brief 地面机器人位置数据

#if ROBOT_TYPE == 4
float shooter_bili1 = 0, shooter_bili2 = 0;
#endif
#if ROBOT_TYPE == 2
float shoot_bili = 0;float shoot_bili0 = 0;
#endif
uint8_t referee_system_buff[USART6_BUFLENx2];
uint8_t referee_system_halfbuff[USART6_BUFLEN];
//Shooter shooter;
...
int referee_system_Rx(uint8_t *RxMsg) {
    PACK_DEAD_t *head = (PACK_DEAD_t *)RxMsg;       // 获取帧头
    uint16_t CMD_code = *(uint16_t *)(RxMsg + 5);   // 获取CMD
    uint8_t *Msg = &RxMsg[7];                       // 获取data
    
    if (head->data_length > USART6_BUFLEN - 9)
        return 0;
    uint16_t Referee_CRC = *(uint16_t *)(RxMsg + 7 + head->data_length);    // 获取CRC16整包校验
    // 包有效，开始解析数据，将数据分配到各个结构体
    if (Referee_CRC == Verify_CRC16_Check_Sum(RxMsg, head->data_length + 7)) {  
        ...
        case CMD_code_Match_status_data:
            ...
            xEventGroupSetBits(EventGroup_Receive_Finish_1, Event_1_Game_Status);
            break;
#endif
...
#if Match_robot_HP_data==REFEREE_SYSTEM_ENABLE  // 比赛机器人血量数据
        case CMD_code_Match_robot_HP_data:
            memcpy(&ext_game_robot_HP, Msg, sizeof(ext_game_robot_HP_t));
            xEventGroupSetBits(EventGroup_Receive_Finish_1, Event_1_Robot_HP);
            break;
#endif
        ...
        case CMD_code_Robot_position_data:
            ...
            xEventGroupSetBits(EventGroup_Receive_Finish_2, Event_2_Sentry_Pose);
            break;
#endif
...
#if Ground_Robot_Position == REFEREE_SYSTEM_ENABLE
        case CMD_code_Ground_Robot_Position:
            memcpy(&ground_robot_position, Msg, sizeof(ground_robot_position_t));
            xEventGroupSetBits(EventGroup_Receive_Finish_2, Event_2_Robot_Pose);
            break;
#endif
        }
...
uint8_t unpack_refree_system_data(RMQueue_Handle *Queue) {
    uint8_t *referee_ptr = RMQueueTop(Queue);
            abc =  RMQueueTop(Queue);
    if (referee_ptr) {
        memcpy(referee_system_buff, referee_system_buff + USART6_BUFLEN, USART6_BUFLEN);
        memcpy(referee_system_buff + USART6_BUFLEN, referee_ptr, USART6_BUFLEN);
        RMQueuePop(Queue);
        for (uint8_t i = 0; i < USART6_BUFLEN; i++) {
...
float refree_shooter_limit_bili() {
    ...
    if(ext_power_heat_data.shooter_id1_17mm_cooling_heat > 
        ext_game_robot_state.shooter_id1_17mm_cooling_limit * shoot_bili0) {
        if(ext_power_heat_data.shooter_id1_17mm_cooling_heat < ext_game_robot_state.shooter_id1_17mm_cooling_limit * shoot_bili)
            return (ext_game_robot_state.shooter_id1_17mm_cooling_limit - ext_power_heat_data.shooter_id1_17mm_cooling_heat)
                / (ext_game_robot_state.shooter_id1_17mm_cooling_limit * 0.3);
        else
            return 0;
    }    
    else 
    ...
    if(ext_power_heat_data.shooter_id1_17mm_cooling_heat > 
        ...
        if(ext_power_heat_data.shooter_id1_17mm_cooling_heat < ext_game_robot_state.shooter_id1_17mm_cooling_limit * 0.8)
